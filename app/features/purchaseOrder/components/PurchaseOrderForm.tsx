...
export default function PurchaseOrderForm({
  ...
  onClose, onSubmit, setForm, handleVendorChange, handleItemChange, handleQtyChange, addItem, removeItem
}: Props) {
  
  const availableBarang = vendorLists.filter(v => v.kodeVendor === form.kodeVendor);

  return (
    <Form open={open} title="Buat Purchase Order Baru" onClose={onClose} onSubmit={onSubmit} saving={saving} error={error} maxWidth="800px">
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "16px", gridColumn: "1 / -1" }}>
        ...
        {/* Kolom Catatan Dihapus dari Sini */}
      </div>

      {form.kodeVendor && (
        <div style={{ gridColumn: "1 / -1", marginTop: "16px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
            <h4 style={{ margin: 0 }}>Daftar Barang (Khusus Vendor: {form.namaVendor})</h4>
                ...
                <div style={{ flex: 2 }}>
                  ...
                  <select className={styles.input} value={item.kodeBarang} onChange={(e) => handleItemChange(idx, e.target.value)}>
                    ...
                    {availableBarang.map(b => (
                      <option key={b.kodeBarang} value={b.kodeBarang}>{b.kodeBarang} - {b.namaBarang}</option>
                    ))}
                  </select>
                </div>
