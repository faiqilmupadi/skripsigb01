...
export default function AdminActivityCard({ activities }: { activities: AdminActivity[] }) {
  return (
    <div style={{ background: "#fff", padding: "24px", borderRadius: "24px", border: "1px solid #f1f5f9", boxShadow: "0 10px 40px -10px rgba(0,0,0,0.03)", height: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: "13px", color: "#64748b", marginTop: "2px" }}>Berdasarkan Nomor Trx (PO & SO)</div>
        </div>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: "20px" }}>
        {activities.map(adm => {
          // [REVISI] Memaksa JavaScript menjadikannya Angka (Number) sebelum ditambah
          const poCount = Number(adm.poCount) || 0;
          const soCount = Number(adm.soCount) || 0;
          const total = poCount + soCount;
          
          const poPercent = total > 0 ? (poCount / total) * 100 : 0;
          const soPercent = total > 0 ? (soCount / total) * 100 : 0;
          ...
          return (
            <div key={adm.userName}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: "14px", marginBottom: "8px", fontWeight: 600, color: "#1e293b" }}>
                ...
                <span style={{ color: "#64748b", fontWeight: 500 }}>{total} Transaksi</span>
              </div>
              <div style={{ height: "12px", background: '#f1f5f9', borderRadius: "10px", overflow: 'hidden', display: 'flex' }}>
                 <div style={{ width: `${poPercent}%`, background: '#3b82f6' }} title={`PO: ${poCount}`} />
                 <div style={{ width: `${soPercent}%`, background: '#22c55e' }} title={`SO: ${soCount}`} />
              </div>
            </div>
          );
        })}
